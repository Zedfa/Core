...
new SepehrModule.MainModule("lookupDirectiveModule", []).addDirective("custLookup", ["$http", "$parse", "$compile", function ($http, $parse, $compile) {
    return {
        ...
        controller: ["$scope", "$element", "$attrs", function ($scope, $element, $attrs) {
            ...
            $scope.getFilterRule = () => {
                ...
                return {
                    ...
                    operator: 'eq',
                    value: $scope.valueName //$scope.lkpItem[$scope.valueName]
                };
            ...
            //$scope.lklEditModeCallback = function (e) {

            //    var item = e._data;
            //    if (item.length == 1) {

            //    }
            //    e.dataSource.filter = initialFilterRule;
        ...
        link: function (scope, elem, attrs, ctrl) {
            ...
            scope.onShortcutCodekeypress = (args, event) => {
            ...
            scope.onShortcutCodeBlur = (args) => {

                scope.filterByField(scope.shortcutCodeName, scope.shortcutCode);
            }

            var listener = scope.$watch("valueName",(current, old) => {
            ...
            scope.filterByField = (fieldName, val) => {
