...
jqueryDateTimeModule.addDirective("jqueryDate", ["$compile", "dateInfoService", function ($compile, dateInfoService) {
        return {
            ...
            controller: ["$scope", "$element", function ($scope, $element) {
                    if (!$scope.selectedDate) {
                        if ($scope.setCurrentDate) {
                            dateInfoService.GetTodaysShamsiDate().then(function (date) {
            ...
            link: function (scope, elem, attrs) {
                ...
                var showButton = !scope.buttonShow || scope.buttonShow === true ? 'button' : 'focus', dFormat = !scope.dateFormat ? 'yy/mm/dd' : scope.dateFormat, yRange = !scope.yearRange ? 'c-90:c+10' : scope.yearRange, maxDate = scope.enableCurrentMonth ? "0" : "";
