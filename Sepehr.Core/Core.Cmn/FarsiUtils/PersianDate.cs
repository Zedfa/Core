...
namespace Core.Cmn.FarsiUtils
{
        ...
        IEquatable<PersianDate>
    {
        ...
        {
            Assign(PersianDateConverter.ToPersianDate(date.Add(utcDifference)));
        }
        public PersianDate()
        {
            year = Now.year;
        ...
        private static PersianDate ParseShortDate(string value)
        {
            ...
            {
                dateParts = value.Split("-".ToCharArray());
            }
        ...
        public string ToString(string format, IFormatProvider formatProvider)
        {
            ...
            switch (format)
            {
                case "D":
                    //'yyyy mm dd dddd' e.g. 'دوشنبه 20 شهریور 1384'
                    return string.Format("{0} {1} {2} {3}", new object[4] { LocalizedWeekDayName, Day, LocalizedMonthName, Year });
                case "SD":
                    //'mm dd dddd' e.g. 'دوشنبه 20 شهریور'
                    return string.Format("{0} {1} {2}", new object[3] { LocalizedWeekDayName, Day, LocalizedMonthName });
                case "DD":
                    //'yyyy mm dd dddd' e.g. 'دوشنبه بیست شهریور هزار و سیصد و هشتاد و چهار'
                    {
                ...
                case "HH:mm":
                    //'hh:mm:ss tt'
                    return string.Format("{0}:{1}", new object[2] { Util.toDouble(Hour), Util.toDouble(Minute) });

                case "yy/MM/dd":
                    return string.Format("{0}/{1}/{2}", Year.ToString().Substring(2), Util.toDouble(Month), Util.toDouble(Day));


                case "YY-MM-DD":
                    return string.Format("{0}-{1}-{2}", Year.ToString().Substring(2), Util.toDouble(Month), Util.toDouble(Day));
                case "YYYY-MM-DD":
                    return string.Format("{0}-{1}-{2}", Year.ToString(), Util.toDouble(Month), Util.toDouble(Day));
                case "d":
                default:
                    //ShortDatePattern yyyy/mm/dd e.g. '1384/9/1'
